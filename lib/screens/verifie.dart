...
class _RegisterPageOdooState extends State<RegisterPageOdoo> {
  ...
  String _formatPhoneNumber(String phoneNumber) {
    ...
    if (!phoneNumber.startsWith('+')) {
      return "+221$phoneNumber"; // Indicatif du Sénégal
    }
  ...
  Future<void> _register() async {
    ...
    try {
      // Récupérer l'API d'authentification depuis le Provider
      ...
      print("Réponse d'enregistrement: $response");
      ...
      if (response != null && response is Map<String, dynamic>) {
        if (response.containsKey('success') && response['success'] == true) {
          // Enregistrement réussi, récupérer l'ID utilisateur
          String userId = '';
          if (response.containsKey('data') &&
              response['data'] is Map<String, dynamic>) {
            userId = response['data']['partner_id'].toString();
          }
          ...
          // Naviguer vers la page de vérification de SMS
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => VerifySmsCodePage(
                phoneNumber: phone,
                userId: userId,
                isRegistration: true,
              ),
